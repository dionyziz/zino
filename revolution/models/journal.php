<?php
	...
	class Journal {
		...
		public static function Item( $id ) {
			$res = db(
					'SELECT
						`bulk_text` as text, `user_name` as username, `user_subdomain` as subdomain, `user_avatarid` as avatarid, `user_gender` as gender, `journal_id` as id, `journal_created` as created, `journal_numcomments` as numcomments, `journal_title` as title, `journal_url` as url, `journal_userid` as userid 
					FROM 
						`journals`
					CROSS JOIN `users` ON

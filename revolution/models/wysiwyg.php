<?php
    ...
    function WYSIWYG_PreProcess( $html ) {
        ...
        $html = preg_replace(
            ...
            '<img src="' . $settings[ 'imagesurl' ] . 'video-placeholder.png?v=$1" />',
            $html
        );
        $html = preg_replace(
            '#\<embed\s*+src\="http\://www\.veoh\.com\/videodetails2\.swf\?permalinkId\=([a-zA-Z0-9_\-]+)[^"]++"[^>]++\>\</embed\>#i',
            '<img src="' . $settings[ 'imagesurl' ] . 'video-placeholder.png?w=$1" />',
            $html
        );
    ...
    function WYSIWYG_PostProcess( $html ) {
        ...
        $html = preg_replace(
           '#\<img[^>]*?src\=([""]?)' 
            . preg_quote( $settings[ 'imagesurl' ], "#i" )

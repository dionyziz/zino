...
if ($_SERVER['REQUEST_METHOD']=='POST')
	...
	if ($step=='get_contacts')
		...
		if (count($ers)==0)
			...
			if (!isset($inviter))
				$ers['inviter']="Invalid email provider";
			elseif (!$inviter->login($_POST['email_box'],$_POST['password_box']))
				$ers['login']="Login failed. Please check the email and password you have provided and try again later";
	...
	elseif ($step=='send_invites')
		...
		if (count($ers)==0)
			...
			if ($sendMessage===-1)
				{
				$message_footer="\r\n\r\nThis invite was sent using OpenInviter technology. If you would like to stop receiving invites sent through OpenInviter visit this link: http://openinviter.com/blacklist_me.php?acct=%s&domain=%s";
				foreach ($selected_contacts as $email=>$name)
					{
					$email=strtolower($email);
					$temp=explode('@',$email);
					$acct=base_convert(md5($temp[0]),16,36);
					$domain=base_convert(md5($temp[1]),16,36);
					$message_subject=$_POST['email_box'].$message['subject'];
					$message_body=$message['body'].$message['attachment'].sprintf($message_footer,$acct,$domain); 
...
$contents="<form action='' method='POST'>".ers($ers).oks($oks);
if (!$done)
	{
	if ($step=='get_contacts')
		{
...
$contents.="<center><a href='http://spread.openinviter.com/click.php?key=ac2b055129a89ee0a07587aac63dc98b'><img src='http://openinviter.com/images/banners/banner_blue_1.gif' border='0' alt='Powered by OpenInviter.com' title='Powered by OpenInviter.com'></a></center>";
if (!$done)
	{

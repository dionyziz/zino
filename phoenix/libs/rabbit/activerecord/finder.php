<?php
    abstract class Finder {
        ...
        protected function FindByPrototype( $prototype, $offset = 0, $limit = 25, $order = false ) {
            ...
            w_assert( is_int( $offset ), 'Finder offset must be an integer in FindByPrototype call in finder `' . get_class( $this ) . '\' ' . gettype( $offset ) . ' given' );
            w_assert( is_int( $limit ), 'Finder limit must be an integer in FindByPrototype call in finder `' . get_class( $this ) . '\', ' . gettype( $limit ) . ' given' );

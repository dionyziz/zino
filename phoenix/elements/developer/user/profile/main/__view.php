<?php
    class ElementDeveloperUserProfileMainView extends Element {
        public function Render( $theuser, $commentid, $pageno ) {
            ...
            if ( $finder->IsFriend( $user, $theuser ) == FRIENDS_B_HAS_A ) {
                Element( 'developer/user/profile/main/antisocial', $theuser );
            }
            ...
            <div class="main"><?php
                ...
                if ( $egoalbum->Numphotos > 0 ) {
                    ?><div class="photos"><?php
                        ...
                        Element( 'developer/user/profile/main/photos' , $images , $egoalbum , $theuser->Id );
                        ?></div><?php
                }
                ?>
                ...
                if ( !empty( $friends ) || ( $user->Id == $theuser->Id && $user->Count->Relations == 0 ) ) { 
                        ...
                        Element( 'developer/user/profile/main/friends' , $friends , $theuser->Count->Relations , $theuser->Id , $theuser->Subdomain , $usernorel );
                    ?><div class="barfade">
                ...
                if ( !empty( $polls ) || ( $user->Id == $theuser->Id && $user->Count->Polls == 0 ) ) {
                    ?><div class="lastpoll">
                        ...
                        if ( $user->Id == $theuser->Id && $user->Count->Polls == 0 ) {
                            ...
                            Element( 'developer/poll/small' , $polls[ 0 ] , true );
                            ?></div><?php
                        }
                    ?></div><?php
                }
                Element( 'developer/user/profile/main/questions' , $theuser );
                ...
                if ( !empty( $journals ) || ( $user->Id == $theuser->Id && $user->Count->Journals == 0 ) ) {
                    if ( $user->Id == $theuser->Id && $user->Count->Journals == 0 ) {
                        Element( 'developer/user/profile/main/lastjournal', false, $theuser, 0, 0, 0, true );
                    }
                    else {
                        Element( 'developer/user/profile/main/lastjournal' , $journals[ 0 ] , $theuser , $journals[ 0 ]->Id , $journals[ 0 ]->Numcomments , $theuser->Count->Journals , false );
                    }
                }
                if ( $user->HasPermission( PERMISSION_COMMENT_VIEW ) ) {
                    ?><div class="comments">
                        ...
                        Element( 'developer/user/name' , $theuser->Id , $theuser->Name , $theuser->Subdomain , false );
                        ?></h3><?php
                        if ( $pageno <= 0 ) {
                            $pageno = 1;
                        }
                            ...
                            if ( $user->HasPermission( PERMISSION_COMMENT_CREATE ) ) {
                                Element( 'developer/comment/reply', $theuser->Id, TYPE_USERPROFILE , $user->Id , $user->Avatarid );
                            }
                        // if ( $theuser->Profile->Numcomments > 0 ) {
                            $indentation = Element( 'developer/comment/arraylist' , $comments , TYPE_USERPROFILE , $theuser->Id );
                            ...
                            if ( $commentid > 0 && isset( $indentation[ $commentid ] ) ) {
                                Element( 'developer/comment/focus', $commentid, $indentation[ $commentid ] );
                            }
                            ?><div class="pagifycomments"><?php
                                $link = str_replace( '*', urlencode( $theuser->Subdomain ), $xc_settings[ 'usersubdomains' ] ) . '?pageno=';
                                Element( 'developer/pagify' , $pageno , $link, $total_pages );
                            ?></div><?php
                        // }
                    ?></div><?php
                }

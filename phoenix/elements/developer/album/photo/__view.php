<?php
    ...
    class ElementDeveloperAlbumPhotoView extends Element {
				 ...
				 if ( $user->Exists() ) {
					...
					if ( $user->Id != $theuser->Id && !$user->HasPermission( PERMISSION_JOURNAL_DELETE_ALL ) ) {
						...
						if ( $image->Album->Ownertype == TYPE_USERPROFILE 
							...
							?><li>
								<a href="" title="Ποιος είναι στην φωτογραφία" onclick="Tag.prestart( false, '', true );return false"><span class="s1_0002">&nbsp;</span>Γνωρίζεις κάποιον;</a>
							</li><?php
						}
					}
					else {
						...
						if ( $image->Album->Ownertype == TYPE_USERPROFILE && $user->Id == $theuser->Id ) {
							if ( $image->Album->Mainimageid != $image->Id ) {
	                            ...
	                            echo $image->Id;
	                            ?>' , this )"><span class="s1_0012">&nbsp;</span>Ορισμός προεπιλεγμένης</a>
	                            </li><?php
	                        }
						}
					}

<?php
    class ElementFooter extends Element {
        public function Render() {
            global $user;
            
            ?><div>
                <a class="wlink" href="contact">Επικοινώνησε</a>
                <a class="wlink" href="tos">Όροι χρήσης</a>
                <a class="wlink" href="?p=ads">Διαφήμιση</a>
            </div>
            <a href="http://dionyziz.zino.gr/journals/Drops_of_Summer" style="position:absolute;right:0;bottom:-3px" title="Zino Meetings: Αθήνα, Βέροια, Θεσσαλονίκη, Γιάννενα">
                <img src="http://static.zino.gr/phoenix/dropsofsummer/footerdrops.png" alt="Drops of Summer" />
            </a>
            <div id="copyleft">
                <span>&copy; 2009</span> <a class="wlink" href="http://www.kamibu.com/">Kamibu</a>
            </div><?php
        }
    }
?>

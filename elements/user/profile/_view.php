		...
		?><div class="userprofile"><br /><br /><br />
			<div class="top"><span>
				...
				if ( $theuser->HasSpecialRank() ) {
					Element( 'user/classavatar', $theuser );
				}
                ...
                <div class="tabs" id="userprofile_tabs">
					...
					if ( !$user->IsAnonymous() && $user->Id() != $theuser->Id() ) { 
                        ...
                        if ( count( $relations ) ) {
                            ...
                            <ul class="daddy" id="frel_type" onmouseover="Friends.ShowAll(true);" onmouseout="Friends.ShowAll(false);"><?php
                            if( $isfriend ) {
                            ...
                            foreach( $relations as $relation ) {
								...
								echo $relation->Id;
								?> );" style="<?php
								if( $counter%2 != 0 && $relation->Id != $relid ) {
									?> background-color: #ececec; <?php
								}
								if( $counter == 0 && $relation->Id != $relid ) {
									?> border-top: thin ridge;<?php
								}
								?>" class="<?php
								echo ( $relation->Id == $relid )?"relselected":"relation";
								?>" <?php
								if( $relation->Id == $relid ) {
									?>title="Επιλεγμένη Σχέση"<?php
								}
								?>><?php
								echo $relation->Type;
								?></li><?php
							...
							?><li id="frel_-1" onclick="Friends.AddFriend( <?php
							echo $theuser->Id();
							?>, -1 );" style="border: thin ridge;<?php
							if( $counter%2 != 0 ) {
								?> background-color: #ececec;<?php
							}
							?>color: #ff0000; <?php
							if( $relid != -1 ) {
								?>opacity: 0;display: none;<?php
							}
							?>" <?php
							if( $relid == -1 ) {
								?>title="Επιλεγμένη Σχέση"<?php
